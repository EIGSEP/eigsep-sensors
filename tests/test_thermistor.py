...
def test_raw_to_temp(therm):
    ...
    therm.set_temp_constants({"A": 1})
    assert therm.raw_to_temp(raw) == 1  # only A is non-zero
    therm.set_temp_constants({"B": 1})
    assert therm.raw_to_temp(raw) == 1 / np.log(expected_rtherm)
    therm.set_temp_constants({"C": 1})
    assert therm.raw_to_temp(raw) == 1 / np.log(expected_rtherm) ** 3
